...
// 	public $host = "http://i2.api.weibo.com/2/";
    /**
     * Set timeout default.
     *
     * @ignore
     ...
     */
    function __construct($client_id, $client_secret, $scope, $callback, $cookie="", $access_token = NULL, $refresh_token = NULL, $host='') {
...
//			$this->host = $host;
        ...
        if ( !empty($config['weibo']['http']['proxy']))
        {
            $this->http_proxy = $config['weibo']['http']['proxy'];
        }
        if(!empty($host))
            $this->host = $host;
        else {
            if ( !empty($config['weibo']['http']['host']) )
            {
                $this->host = $config['weibo']['http']['host'];
            }
        }
    }


    public function setUseI2 ( $useI2 )
    {
        if ( !$useI2 )
        {
            $this->host = "https://api.weibo.com/2/";
     ...
     * @return array
     */
    function getAuthorizeURL($state = NULL, $response_type = 'code', $display = NULL ) {
        $params = array();
    ...
    {
        $this->get("account/end_session",['access_token'=>$this->access_token]);
    }
...
class SaeTClientV2
{
     ...
     * @return void
     */
    function __construct( $akey, $skey, $scope='', $callback='', $cookie='', $access_token, $refresh_token = NULL, $host = '')
    {
        $this->oauth = new SaeTOAuthV2( $akey, $skey, $scope, $callback, $cookie, $access_token, $refresh_token ,$host);
    }
     ...
     * 将一个或多个长链接转换成短链接
     *
     * 对应API：{@link http://open.weibo.com/wiki/2/short_url/shorten}
     *
     * @param string $url 需要转换的长链接，需要URLencoded，。
     * @return array
    {
    "urls": [
    {
     ...
     * @ignore
     */
    function isCookiesAvailable($id)
    {
        $user = $this->account_profile_basic();
        if (!$user || isset($user['error_code']))
        {
            return false;
        }
//        $user = $this->show_user_by_id($id);
