...
function arrayCol($array, $colName)
{
    $newArray = array();
    foreach ($array as $v) {
        $newArray[] = $v[$colName];
...
function arraySearchCol($array, $colName, $key)
{
...
function convertToArrayN($input)
{
    if (!isset($input))
        return $input;
...
function ensureFilePath($file_path, $is_dir = false)
{
 ...
 */
function halo_array_merge($array1, $array2 = null, $_ = null)
{
    $varArray = func_get_args();
    $count = count($varArray);
...
function connectQuery($path, $params)
{
    if (empty($params)) {
        return $path;
    }
...
function unsetUrlParam($url, $key)
{
...
function resetUrlParam($url, $param)
{
    $urlObj = parse_url($url);
    $params = convertQuery($urlObj['query']);
...
function addUrlParamFromUrl($oriUrl, $toUrl)
{
...
function getFromServer($key, $default = null)
{
    return (isset($_SERVER[$key])) ? $_SERVER[$key] : $default;
}

function getRequestUrl() {
    return sprintf('%s http://%s%s %s',getFromServer('REQUEST_METHOD'), getFromServer('HTTP_HOST'), getFromServer('REQUEST_URI'), getFromServer('SERVER_PROTOCOL'));
}
...
function generateRandomStr($len = 40)
{
...
function checkUrlImageExist($url)
{
    $ch = curl_init();
