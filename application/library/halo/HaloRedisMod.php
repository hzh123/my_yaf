...
class HaloRedis
{
    const USER_INTEREST_SETTING = "user_interest_setting";
    const USER_MANAGE_WIZARD = "user_manage_wizard";
    const USER_INVITE_SHARE_WIZARD = "user_invite_share";

    const PINYIN_TABLE = "pinyin_table";
    //REDIS服务主机IP
    private $_HOST = null;
    ...
    //初始化
    public function __construct($host, $port, $password = null, $timeout = 0, $dbname = "", $type = 1)
    {
//        $this->_HOST = REDIS_HOST;
//        $this->_PORT = REDIS_PORT;
//        $this->_TIMEOUT = REDIS_TIMEOUT;
//        $this->_DBNAME = REDIS_DBNAME;
//        $this->_CTYPE = REDIS_CTYPE;
    ...
    /**
     * 查看redis连接是否断开
     * @return bool $return true:连接未断开 false:连接已断开
     */
     ...
     * @return bool $return true:成功 false:失败
     */
    public function set($key, $value, $type = 0, $repeat = 0, $time = 0.0, $old = 0)
    {
        $return = null;
    ...
    public function get($key = null, $start = null, $end = null)
    {
        ...
        if (is_array($key) && !empty($key)) {
            ...
            if (isset($start) && isset($end)) $return = $this->_REDIS->getRange($key, $start, $end);
     ...
     * @param int|string $direction 0:数据入队列头(左) 1:数据入队列尾(右) 默认为0
...
//    }
     ...
     * @return array
     */
    public function setRangeByScore($set, $start, $end, $order = 0, $score = false)
    {
        $return = null;

        if ($order) {
            $return = $this->_REDIS->zRangeByScore($set, $start, $end, ['withscores' => $score]);
        } else {
            $return = $this->_REDIS->zRevRangeByScore($set, $start, $end, ['withscores' => $score]);
        }
     ...
     * @param string $set 集合名
     * @param int|string $start 开始score 或者 -inf
     * @param int|string $end 结束score 或者 +inf
     * @return int
     */
     ...
     * @return array|string
     */
    public function hashGet($hash, $key = null, $type = 0)
    {
        $return = null;
    ...
    /**
     * 获取满足给定pattern的所有key
     * @param string $key key匹配表达式 模式:user* 匹配以user开始的key
     * @return array
     */
